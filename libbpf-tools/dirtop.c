...
static int print_stat(struct dirtop_bpf *obj) {
        ...
        printf("%-7s %-16s %-6s %-6s %s %s\n", "READS", "WRITES", "R_Kb", "W_Kb", "PATH", "FILE");
        while (1) {
...
int main(int argc, char **argv) {
        ...
        if (target_pid)
                obj->rodata->target_pid_value = target_pid;
